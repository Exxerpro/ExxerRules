...
/// <summary>
/// Represents the PlcVirtualRepository.
/// </summary>

public class PlcVirtualRepository(IOptions<PlcDbOptions> options, ILogger<PlcDbTagsRepository> logger, IDbConnection db) : IPlcDBRepository

{
    private readonly string connectionString = options.Value.ConnectionString;

    /// <summary>
    /// Retrieves all virtual PLCs asynchronously from the database.
    /// </summary>
    ...
    public async Task<IEnumerable<PlcData>> GetPlcsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var plcs = await db.QueryAsync<PlcData>(
                "SELECT PlcId, Enabled, MachineId, Name, IpAddress, PlcType, PlcBrand FROM Plcs", cancellationToken);
            return plcs;
        }
    ...
    public async Task<IEnumerable<Db>> GetDistinctDbInfosAsync(CancellationToken cancellationToken)
    {
        try
        {
            var sql = @"
			SELECT DISTINCT
				CAST(SUBSTRING(Address, 3, CHARINDEX('.', Address) - 3) AS INT) AS UserId
			FROM Variables";
        ...
        CancellationToken cancellationToken)
    {
        try
        {
			...
			SELECT MachineId, Name, Address, NetType, 0 as ValueInt, 0 as ValueReal, 0 as ValueBool, '' as ValueString
			FROM Variables
			WHERE MachineId IN @machineId AND And groupId=@groupId AND IsActive = 1 ";
            // Using Dapper's parameterized query to prevent SQL injection and handle the list of machineId
            var command = new CommandDefinition(
                sql,
