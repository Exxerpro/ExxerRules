...
internal static class ValueConverter
{
    private static readonly Dictionary<Type, WriteFunc> WriteFunctions = new()
    {
        ...
        { typeof(double), (data, _, value) => BinaryPrimitives.WriteDoubleBigEndian(data, (double)value) },
        {
            typeof(string), (data, address, value) =>
            {
                ...
                {
                    throw new ArgumentException("ValueString must be of type string", nameof(value));
                }
    ...
    public static bool TryWriteToBuffer(Span<byte> buffer, object value, Type type, VariableAddress address)
    {
        try
        {
            if (type == typeof(bool))
            {
                WriteToBuffer(buffer, (bool)value, address);
            }
            else if (type == typeof(byte))
            {
                WriteToBuffer(buffer, (byte)value, address);
            }
            else if (type == typeof(short))
            {
                WriteToBuffer(buffer, (short)value, address);
            }
            else if (type == typeof(ushort))
            {
                WriteToBuffer(buffer, (ushort)value, address);
            }
            else if (type == typeof(int))
            {
                WriteToBuffer(buffer, (int)value, address);
            }
            else if (type == typeof(uint))
            {
                WriteToBuffer(buffer, (uint)value, address);
            }
            else if (type == typeof(long))
            {
                WriteToBuffer(buffer, (long)value, address);
            }
            else if (type == typeof(ulong))
            {
                WriteToBuffer(buffer, (ulong)value, address);
            }
            else if (type == typeof(float))
            {
                WriteToBuffer(buffer, (float)value, address);
            }
            else if (type == typeof(double))
            {
                WriteToBuffer(buffer, (double)value, address);
            }
            else if (type == typeof(string))
            {
                WriteToBuffer(buffer, (string)value, address);
            }
            else
            {
