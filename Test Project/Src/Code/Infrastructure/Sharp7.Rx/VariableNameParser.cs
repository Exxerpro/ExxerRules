// <copyright file="VariableNameParser.cs" company="PlaceholderCompany">
...
/// <summary>
/// Provides parsing functionality for S7 variable addresses from string representations.
/// </summary>
internal class VariableNameParser : IVariableNameParser
{
    private static readonly Regex Regex = new(
        @"^(?<operand>db)(?<dbNo>\d+)\.?(?<type>[a-z]+)(?<start>\d+)(\.(?<bitOrLength>\d+))?$",
    ...
    /// <summary>
    /// Parses a string input into a <see cref="VariableAddress"/>.
    /// </summary>
    /// <param name="input">The string representation of the variable address.</param>
    /// <returns>A <see cref="VariableAddress"/> parsed from the input string.</returns>
    /// <exception cref="InvalidS7AddressException">Thrown if the input is not a valid S7 address.</exception>
