...
/// <summary>
/// Unit tests for GetBarCodeListQueryReportsValidator
/// </summary>
public class GetBarCodeListQueryReportsValidatorTests
{
    //[Fix]
    //CLAUDE
    //Date: 21/08/2025
    //Reason: Replaced placeholder tests with comprehensive validation tests for GetBarCodeListQueryReportsValidator

    private readonly GetBarCodeListQueryReportsValidator _validator = null!;
    private readonly DateTime _validDate = DateTime.Now.AddDays(-1);
    private readonly DateTime _futureDate = DateTime.Now.AddDays(2);

    public GetBarCodeListQueryReportsValidatorTests()
    {
        _validator = new GetBarCodeListQueryReportsValidator();
    }
    ...
    public void Should_Have_Error_When_StartDate_Is_Null()
    {
        ...
        //CLAUDE
        //Date: 21/08/2025
        //Reason: Fixed test - DateTime struct cannot be null, so test should check for MinValue or future date validation instead
        var query = new GetReportsListQuery { StartDate = DateTime.MinValue, EndDate = _validDate };
        ...
        // DateTime.MinValue triggers validation because it's effectively an invalid date for business logic
    ...
    public void Should_Have_Error_When_EndDate_Is_Null()
    {
        ...
        //CLAUDE
        //Date: 21/08/2025
        //Reason: Fixed test - DateTime struct cannot be null, so test should check for MinValue validation
        var query = new GetReportsListQuery { StartDate = _validDate, EndDate = DateTime.MinValue };
        var result = _validator.TestValidate(query);
        //[Fix]
        //CLAUDE
        //Date: 21/08/2025
