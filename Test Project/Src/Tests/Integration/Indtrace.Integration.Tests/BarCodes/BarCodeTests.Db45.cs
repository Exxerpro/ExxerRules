...
public class BarCodeTests_Db45 : IClassFixture<Integration.Tests.Infrastructure.TestHostFixture>
{
    private readonly IServiceProvider _services;
    private readonly ITestOutputHelper _output;

    public BarCodeTests_Db45(Integration.Tests.Infrastructure.TestHostFixture fixture, ITestOutputHelper output)
    {
    ...
    public static IEnumerable<object[]> Cases => Integration.Tests.Data.BarCodeCases_Db45.Cases;

    [Theory(Skip = "Missing DB or SKIP_DB_TESTS set.", SkipWhen = nameof(ShouldSkipDb45))]
    [MemberData(nameof(Cases))]
    [Trait("Db", "DB45")]
    public async Task Should_Find_Label_On_DB45(string barCode, int machineId, int _)
    {
        const string dbKey = Integration.Tests.Utilities.DbProfiles.IndTraceDbContext45;

        using var scope = _services.CreateScope();
        DbLogging.LogConnectionString(scope.ServiceProvider, dbKey, _output, nameof(BarCodeTests_Db45));
    ...
    [MemberData(nameof(Cases))]
    [Trait("Db", "DB45")]
    public async Task Should_Have_Valid_BarCodeId_On_DB45(string barCode, int machineId, int expectedBarCodeId)
    {
        const string dbKey = Integration.Tests.Utilities.DbProfiles.IndTraceDbContext45;
        ...
        // Test BarCodeId assertion separately
