...
public class BarCodeTests_QA46 : IClassFixture<Integration.Tests.Infrastructure.TestHostFixture>
{
    private readonly IServiceProvider _services;
    private readonly ITestOutputHelper _output;

    public BarCodeTests_QA46(Integration.Tests.Infrastructure.TestHostFixture fixture, ITestOutputHelper output)
    {
        _services = fixture.Services;
        _output = output;
    }

    public static IEnumerable<object[]> Cases => Integration.Tests.Data.BarCodeCases_QA46.Cases;

    public static bool ShouldSkipQA46 => TestDbGuards.ShouldSkipQA46;

    [Theory(Skip = "Missing QA46 DB or SKIP_DB_TESTS set.", SkipWhen = nameof(ShouldSkipQA46))]
    [MemberData(nameof(Cases))]
    [Trait("Db", "QA46")]
    public async Task Should_Find_Label_On_QA46(string barCode, int machineId, int expectedBarCodeId)
    {
        const string dbKey = Integration.Tests.Utilities.DbProfiles.IndTraceDbContext46;

        using var scope = _services.CreateScope();
        DbLogging.LogConnectionString(scope.ServiceProvider, dbKey, _output, nameof(BarCodeTests_QA46));
