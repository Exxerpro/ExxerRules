...
public class UseXUnitV3Analyzer : DiagnosticAnalyzer
{
    ...
    private static bool IsIgnoredByProjectConfig(SyntaxNodeAnalysisContext context)
    {
        ...
        if (!string.IsNullOrEmpty(path) && path.IndexOf(".Analyzer.Tests", StringComparison.OrdinalIgnoreCase) >= 0)
        {
            return true;
        }
        ...
        {
            var parts = (list ?? string.Empty).Split(new[] { ';', ',' }, StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
