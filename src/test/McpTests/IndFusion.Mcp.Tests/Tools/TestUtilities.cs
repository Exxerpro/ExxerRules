...
""";
    ...
    private static void EnsureTestSolution(string solutionPath)
    {
        ...
        var exampleCodePath = Path.Combine(projectDir, "ExampleCode.cs");
        
        // Always ensure ExampleCode.cs exists, even if solution already exists
        if (!File.Exists(exampleCodePath))
        {
            Directory.CreateDirectory(projectDir);
            var exampleCodeContent = """
                using System;
                using System.Collections.Generic;
            ...
            File.WriteAllText(exampleCodePath, exampleCodeContent);
        }
